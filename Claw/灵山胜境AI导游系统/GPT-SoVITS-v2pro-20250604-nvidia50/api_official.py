...
def get_tts_wav(
    ...
    if spk not in speaker_list:
        logger.error(f"speaker_list keys: {list(speaker_list.keys())}, requested: {spk}")
        raise Exception(f"Speaker {spk} not found in speaker_list. Available: {list(speaker_list.keys())}")
