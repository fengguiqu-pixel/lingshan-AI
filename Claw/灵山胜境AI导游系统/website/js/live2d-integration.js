...
;(function () {
  ...
  async function loadLive2DConfig () {
    try {
      // 加载表情映射
      const exprResp = await fetch('http://localhost:3001/api/character/expressions')
      if (exprResp.ok) {
      ...
      // 加载口型配置
      const lipResp = await fetch('http://localhost:3001/api/character/lip-sync')
      if (lipResp.ok) {
      ...
      // 加载动作配置
      const motResp = await fetch('http://localhost:3001/api/character/motion-config')
      if (motResp.ok) {
