...
;(function () {
  ...
  async function onAllLoaded () {
    ...
    // 通过 http:// 协议加载模型
    try {
      // 增加 15 秒加载超时保护（移动端大模型可能加载慢）
      await Promise.race([
        loadModel(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('模型加载超时（15秒）')), 15000))
      ])
      isModelLoaded = true
  ...
  function showLoadError (err) {
    ...
    console.warn('[Live2D] 降级显示静态头像，原因:', msg)
    // 不覆盖容器，让 showFallbackAvatar 显示头像；只在控制台记录错误
  }
  ...
  function showFallbackAvatar () {
    ...
    if (!container) return
    container.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:8px;color:#999;padding:12px;text-align:center;">
        <img src="assets/march7th-avatar.png" alt="小灵" style="width:110px;height:110px;border-radius:50%;object-fit:cover;border:2px solid rgba(200,163,87,0.3);">
        <div style="font-size:14px;font-weight:600;color:var(--primary);">小灵</div>
        <div style="font-size:11px;color:var(--text-secondary);">AI 数字人导游</div>
        <button class="l2d-retry-btn" onclick="location.reload()" style="margin-top:10px;padding:6px 16px;background:var(--primary);color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;">重新加载模型</button>
      </div>
    `
  }
