...
function renderHome() {
  ...
  scroll.innerHTML = SCENIC_SPOTS.map((spot, i) => `
    <div class="spot-card" onclick="openDetail(${i})">
      <div class="spot-card-img" style="background-image:linear-gradient(to top, rgba(0,0,0,0.78) 0%, rgba(0,0,0,0.25) 45%, transparent 100%), url('${spot.heroImage}')">
  ...
  // AR 旅游横向滚动
  const arScroll = document.getElementById('arTourScroll');
  if (arScroll) {
    arScroll.innerHTML = SCENIC_SPOTS.map((spot, i) => `
      <div class="ar-tour-card" onclick="openARTour(${i})">
        <div class="ar-tour-card-img" style="background-image:url('${spot.heroImage}')">
          <div class="ar-tour-card-badge">🥽 AR</div>
          <div class="ar-tour-card-play">▶</div>
        </div>
        <div class="ar-tour-card-body">
          <div class="ar-tour-card-name">${spot.name}</div>
          <div class="ar-tour-card-tag">${spot.tag}</div>
        </div>
      </div>
    `).join('');
  }
